...
1. Run `ERSCLauncher-1.1.1.exe` on Windows 10 or 11.
...
The output is `dist\ERSCLauncher.exe`; the release copy is named `dist\ERSCLauncher-1.1.1.exe`. It contains the .NET runtime and needs no separate runtime installation. The launcher downloads the mod directly from the release asset; the mod files are not bundled into the EXE.
