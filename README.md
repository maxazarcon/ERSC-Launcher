...
1. Run `ERSCLauncher.exe` on Windows 10 or 11.
...
The output is `dist\ERSCLauncher.exe`. It contains the .NET runtime and needs no separate runtime installation. The launcher downloads the mod directly from the release asset; the mod files are not bundled into the EXE.
